...
func DefaultModel() *Model {
	...
	return &Model{
		...
		Commands: []CommandSpec{
			...
			{Name: CommandSetup, Path: []string{"setup"}, Usage: "engram setup [--harness HARNESS] [--project PATH] [--memory-file FILE] [--check-history] [--dry-run]", Summary: "Converge project memories and agent-harness integration", Positionals: positionals(0, 0), Options: []OptionSpec{enumValue("harness", "HARNESS", []string{"claude-code", "codex"}, "--harness"), value("project", "PATH", "--project"), value("memory-file", "FILE", "--memory-file"), flag("check-history", "--check-history"), dryRun}, Store: StoreForbidden},
			...
			{Name: CommandCheck, Path: []string{"check"}, Usage: "engram check [PATH]\nengram check --accepted\nengram check --history\nengram check --staged\nengram check --base BASE --candidate CANDIDATE", Summary: "Validate a snapshot, candidate, or transition", Positionals: positionals(0, 1, "PATH"), Options: []OptionSpec{flag("accepted", "--accepted"), flag("history", "--history"), flag("staged", "--staged"), value("base", "BASE", "--base"), value("candidate", "CANDIDATE", "--candidate")}, Store: StoreConditional, Validate: validateCheck},
