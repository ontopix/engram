...
func TestParseCompleteCommandSurface(t *testing.T) {
	...
	tests := []struct {
		...
		{"setup", []string{"setup", "--harness", "codex", "--memory-file", "MEMORY.md", "--check-history", "--dry-run"}, CommandSetup},
		...
		{"check history", []string{"check", "--history"}, CommandCheck},
...
func TestParseRejectsInvalidGrammar(t *testing.T) {
	...
	tests := []struct {
		...
		{"check history mixed", []string{"check", "--accepted", "--history"}, "mutually exclusive"},
